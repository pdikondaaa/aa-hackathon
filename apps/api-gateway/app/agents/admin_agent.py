...
def admin_agent(query: str) -> str:
    """Admin Agent — retrieves answers from the vector DB."""
    chunks = _retrieve(query, top_k=5)

    if not chunks:
        return (
            f"I couldn't find administrative information for '{query}' in the knowledge base. "
            "Please contact Administration at admin@company.com for assistance."
        )
