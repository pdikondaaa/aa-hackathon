...
def is_reachable(url: str, timeout: int = 5) -> bool:
    """TCP probe — works for both http (port 11434) and https (port 443) URLs."""
    parsed = urlparse(url)
...
def build_chat_llm(
    ...
    """
    Return (ChatOllama, url, model) using the primary endpoint if reachable,
    otherwise the fallback. Returns (None, None, None) if both are down.

    Primary gets the API key header; fallback (ml01) has no auth.
    """
    ...
    for url, model, key in candidates:
        ...
        try:
            ...
            print(f"[LLMFactory] Could not init ChatOllama at {url}: {exc}")
