...
Wiki layout (under apps/var/wiki/):
...
def read_page(rel_path: str) -> str:
    """Read a wiki page by path relative to wiki_root(). Returns '' if missing."""
...
def upsert_index_entry(rel_path: str, title: str, summary: str) -> None:
    ...
    lines = [l for l in index.splitlines() if f"]({rel_path})" not in l]
    lines.append(entry_line)
    if not index:
        header = "# AURA Wiki Index\n\n_Auto-maintained by WikiAgent._\n\n"
...
def list_pages(category: Optional[str] = None) -> List[str]:
    """Return rel_paths of all .md pages, optionally filtered by category."""
    root = wiki_root()
