...
def create_pii_rule(body: CreateRuleIn, ctx: dict = Depends(jwt_auth)):
...
def test_pii_rule(body: TestRuleIn, _ctx: dict = Depends(jwt_auth)):
...
def update_pii_rule(id: str, body: UpdateRuleIn, ctx: dict = Depends(jwt_auth)):
...
def get_pii_log(id: str, _ctx: dict = Depends(jwt_auth)):
...
def review_pii_log(id: str, body: ReviewLogIn, ctx: dict = Depends(jwt_auth)):
    """Reviewer marks a redaction event as FP or TP, with optional review notes."""
    log = _service.review_log(id, ctx["claims"]["sub"], body.is_false_positive, body.review_notes)
    if not log:
        raise HTTPException(status_code=404, detail="PII log not found")
    return log
...
def pii_analytics(_ctx: dict = Depends(jwt_auth)):
