...
from app.api.services.coo_analytics_service import get_coo_dashboard, get_filter_options, get_raw_records
...
async def coo_raw_records(
    ...
    allocation_filter:  Optional[str] = Query(None),
    current_user: dict = Depends(get_current_user),
):
    """Returns raw allocation_details rows for the selected group/filter. Max 500 rows."""
    filters = {k: v for k, v in {
        "function": function, "subfunction": subfunction, "client": client,
        "project_status": project_status, "delivery_manager": delivery_manager,
