...
def submit_feedback(id: str, body: SubmitFeedbackIn, ctx: dict = Depends(jwt_auth)):
    """Thumbs up/down with optional category and comment for an assistant message."""
    result = _service.submit_feedback(
        id, ctx["claims"]["sub"], body.rating, body.category, body.comment
    )
    if result is None:
...
def update_feedback(id: str, body: UpdateFeedbackIn, ctx: dict = Depends(jwt_auth)):
