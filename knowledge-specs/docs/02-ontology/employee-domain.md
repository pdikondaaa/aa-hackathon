...
| Semantic skill matching | Use the 768-dimensional pgvector embeddings to match skill queries semantically rather than via ILIKE pattern matching | Embed `SkillSet` values at ingestion time and store in `document_chunks` with `source = 'employee_skill'` tag |
