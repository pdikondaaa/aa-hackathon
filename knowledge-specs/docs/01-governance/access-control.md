...
The MasterAgent (supervisor_agent.py) enforces agent-level access before routing queries. The permission check occurs in the supervisor's routing phase — regex fast-paths followed by keyword scoring against `DOMAIN_KEYWORDS` (an LLM-based classification method exists in code but is not currently invoked) — ensuring no privilege escalation through prompt manipulation.
...
| HRAgent | `hr`, `admin` | Keyword-scoring fallback (`DOMAIN_KEYWORDS`) |
| ITAgent | `it`, `admin` | Keyword-scoring fallback (`DOMAIN_KEYWORDS`) |
| AdminAgent | `admin` | Keyword-scoring fallback (`DOMAIN_KEYWORDS`) |
| OrgAgent | `org`, `admin` | Keyword-scoring fallback (`DOMAIN_KEYWORDS`) |
| PMOAgent | `admin`, `executive`, `business_lead` | Keyword-scoring fallback; PMO keywords |
| FinanceAgent | `admin` | Keyword-scoring fallback; finance keywords |
...
The DocumentAgent supports generation of 12 HR document types (plus a free-text custom mode). Access is restricted to prevent unauthorized generation of legally binding organizational documents.

### Document Generation Access

| Document Type | Minimum Role | Notes |
|---|---|---|
| Loan Proof Letter | `hr` | |
